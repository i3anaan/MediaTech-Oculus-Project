...
public class OVRCameraRig : MonoBehaviour
{
    ...
    public bool updateTranslation = true;
    ...
    private void UpdateAnchors()
    {
        ...
        if (updateRotation)
        {
            setRotation(leftEye.orientation, leftEye.orientation, rightEye.orientation);
        }
        if (updateTranslation)
        {
            setPosition(leftEye.position, 0.5f * (leftEye.position + rightEye.position), rightEye.position);
        }
    }

    public void setRotation(Quaternion left, Quaternion mid, Quaternion right)
    {
        leftEyeAnchor.localRotation = left;
        centerEyeAnchor.localRotation = mid; // using left eye for now
        rightEyeAnchor.localRotation = right;
    }
    public void setPosition(Vector3 left, Vector3 mid, Vector3 right)
    {
        leftEyeAnchor.localPosition = left;
        centerEyeAnchor.localPosition = mid;
        rightEyeAnchor.localPosition = right;
    }
